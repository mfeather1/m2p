...
function animcube_params(n) {
  ...
  if (typeof n != 'undefined') {
    if (typeof initmove != 'undefined')    p += '&initmove=' + initmove;
    if (typeof initrevmove != 'undefined') p += '&initrevmove=' + initrevmove;
  }
  return p;
  /* these params are not allowed:
     buttonbar, colorscheme, colors, config, demo, gabbacolors,
     initrevmove, move, pos, randmoves, scramble, scw,
...
function show_log() { 
  ...
  if (!(typeof fse == 'undefined' || fse == null)) {
    ...
    </head><body style="color:#ffffff; background-color:#38383D;
    margin-left:10%; margin-right:10%">`;
    s += title + '<br>';
    for (var i=0; i < logtxt.length; i++)
      s += logtxt[i] + '\n';
    s += `<button style="height:28px; width:100px; border-radius:15px;
    background:white; padding:0px; border:none;"
