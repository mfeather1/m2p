...
function AnimCube3(params) {
  ...
  */
  ...
  function mousedown(e) {
    ...
    var cx = (x - left) * dpr;
    var cy = (y - top) * dpr;
    var pc = document.getElementById('paint_cube').checked;
    if (pc && typeof selectedColor != 'undefined') {
      for (var i=0, mod=0; i < faceletAreasIndex; i++) {
        var fa = faceletAreas[i];
        if (pnpoly(fa[0], fa[1], cx, cy)) {
          cube[fa[2]][fa[3]] = selectedColor;
          initialCube[fa[2]][fa[3]] = selectedColor;
          paint();
          upd_layout();
          break;
        }
